...
const ProductDetail = () => {
  ...
  const { thumbnail, title, description, category, price, images } = data;
  return (
        ...
        <article className="mx-auto w-full block lg:flex mt-4 h-full 2xl:h-[70vh]  shadow-lg border rounded-md duration-300 hover:shadow-sm">
          ...
          <div className="w-full lg:w-5/12 flex flex-col justify-evenly p-4">
            ...
            <div className="flex justify-end gap-3 mt-4">
              <button className="border rounded-lg bg-labelColor text-white p-2" onClick={()=> navigate(-1)}>
                Geri
              </button>
              <button className="border rounded-lg bg-main text-white p-2" onClick={()=> navigate("/dashboard")}>
                Ana Sayfaya Dön
              </button>
            </div>
          </div>
